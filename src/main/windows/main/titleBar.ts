// Copyright (c) 2020-2026 grommunio GmbH. All Rights Reserved.

import { WebContentsView, WindowOpenHandlerResponse } from 'electron'

import { getAppPath } from '../../utils/paths'
import { TITLE_BAR } from '../../../constants/window'
import { DEV_TOOLS_OPTIONS, DEV_SERVER_BASE_URL } from '../../constants/view'
import { View } from 'src/main/types/misc'
import { ON_APP_MENU_CLOSE, ON_SERVER_SWITCH } from '../../constants/communication'
import { Server } from '../../../types/misc'

export default class TitleBarView implements View {
  private static readonly DEFAULT_HTML_FILE = 'main-titleBar.html'
  private view?: WebContentsView
  private isProduction: boolean
  private isMenuOpen: boolean
  private lastContentSize?: number[]
  private currentServer?: Server

  constructor(isProduction: boolean) {
    this.isProduction = isProduction
    this.isMenuOpen = false
  }

  private load = (): void => {
    if (this.view == null)
      return

    if (this.isProduction)
      this.view.webContents.loadFile(getAppPath(TitleBarView.DEFAULT_HTML_FILE))
    else
      this.view.webContents.loadURL(`${DEV_SERVER_BASE_URL}${TitleBarView.DEFAULT_HTML_FILE}`)
  }

  private registerListeners = (): void => {
    if (this.view == null)
      return

    this.view.webContents.on('did-finish-load', () => {
      this.sendServer()
    })

    this.view.webContents.on('page-title-updated', (e) => {
      e.preventDefault()
    })

    this.view.webContents.on('will-prevent-unload', (e) => {
      e.preventDefault()
    })

    this.view.webContents.setWindowOpenHandler((): WindowOpenHandlerResponse => {
      return { action: 'deny' }
    })
  }

  adjustBounds = (contentSize: number[]): void => {
    if (this.view == null)
      return

    this.lastContentSize = contentSize
    this.view.setBounds({
      x: 0,
      y: 0,
      width: contentSize[0],
      height: TITLE_BAR.HEIGHT + (this.isMenuOpen ? TITLE_BAR.MENU_OVERLAY_HEIGHT : 0),
    })
  }

  create = (contentSize: number[]): WebContentsView => {
    if (this.view != null)
      return this.view

    this.view = new WebContentsView({
      webPreferences: {
        preload: getAppPath('preload.js'),
      },
    })
    this.view.setBackgroundColor('#00000000')

    this.registerListeners()
    this.adjustBounds(contentSize)
    this.load()

    return this.view
  }

  close = (): void => {
    if (this.view == null)
      return
    this.view.webContents.close()
    this.view = undefined
  }

  toggleDevTools = (): void => {
    if (this.view == null)
      return
    if (!this.view.webContents.isDevToolsOpened())
      this.view.webContents.openDevTools(DEV_TOOLS_OPTIONS)
    else
      this.view.webContents.closeDevTools()
  }

  getWebView = (): WebContentsView => {
    if (this.view == null)
      throw new Error('TitleBar view is not initialized')
    return this.view
  }

  // IPC functions
  sendAppMenuClose = (): void => {
    this.view?.webContents.send(ON_APP_MENU_CLOSE)
  }

  setServer = (server: Server): void => {
    this.currentServer = server
    this.sendServer()
  }

  private sendServer = (): void => {
    this.view?.webContents.send(ON_SERVER_SWITCH, this.currentServer)
  }

  setMenuOpen = (isOpen: boolean): void => {
    this.isMenuOpen = isOpen
    if (this.lastContentSize != null)
      this.adjustBounds(this.lastContentSize)
  }
}
